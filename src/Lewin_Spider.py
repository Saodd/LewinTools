...
class Lewin_Spider:
    ...
    def to_file(self, path_text="", path_cont="", dirname=""):
        default_text = 'spider_%s_%s.txt' % (self.url.replace("http://", "").replace(".","_")[:15], datetime.now().strftime("%H%M%S"))
        default_cont = 'spider_%s_%s.bin' % (self.url.replace("http://", "").replace(".","_")[:15], datetime.now().strftime("%H%M%S"))
...
#     p.join()
    ...
    def post_threading(self, q_post:Queue, q_back:Queue, q_log:Queue):
        ...
        ts = []
        for i in range(7):
            t = threading.Thread(target=Lewin_Spider_Threading._post_worker, args=(q_post, q_back, q_log, "Thread-%d"%i))
            ts.append(t)
        for t in ts:
            t.start()
        for t in ts:
    ...
    @staticmethod
    def post_processor(func, q_back: Queue, q_result: Queue, q_log: Queue, name: str="unnamed-processor", *args, **kwargs):
